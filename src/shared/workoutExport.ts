...
export function formatWorkoutPlanText(plan: WorkoutPlan, exercises: Exercise[], includeSteps = false, programs: readonly WorkoutProgram[] = []): string {
  ...
  return [
    ...
    ...(plan.constraints?.durationMinutes ? [`Time available: ${plan.constraints.durationMinutes} minutes`] : []),
    ...(plan.constraints?.availableEquipment?.length ? [`Available machines: ${plan.constraints.availableEquipment.join(', ')}`] : []),
    '',
    '*Workout*',
    ...exerciseLines,
